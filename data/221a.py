...
def get_args():
    ...
    parser.add_argument(
        "-k", "--gmaps-key", help="Google Maps Javascript API Key", required=True
    )
    parser.add_argument(
        "-C",
        "--cors",
    ...
    parser.add_argument(
        "-t", "--num-threads", help="Number of search threads", type=int, default=1
    )
    parser.add_argument(
        "-np",
        "--no-pokemon",
    ...
    if args.only_server:
        ...
        if args.username is None or args.location is None or args.step_limit is None:
            ...
            print(
                sys.argv[0]
                + ": error: arguments -u/--username, -l/--location, -st/--step-limit are required"
            )
            sys.exit(1)
...
def insert_mock_data():
    ...
    latitude, longitude = float(config["ORIGINAL_LATITUDE"]), float(
